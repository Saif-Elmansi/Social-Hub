...
export default function Register() {
  ...
  async function sendDataToRegister(values) {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/users/signup`, values);
          ...
          <div className="card bg-white w-full max-w-xl p-8 shadow-xl rounded-xl">
            ...
            >
              ...
              <div className="mb-3">
                ...
                <input {...register("name")} type="text" className="input w-full p-2 border rounded" placeholder="username" id="name" />
              ...
              <div className="mb-3">
                ...
                <input {...register("email")} type="text" className="input w-full p-2 border rounded" placeholder="email" id="email" />
              ...
              <div className="mb-3">
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-[50%] end-3 translate-y-[-50%] cursor-pointer text-gray-500">
                    {showPassword ? <Eye onClick={handlePassword} size={20}/> : <EyeOff onClick={handlePassword} size={20}/>}
                  </div>
                </div>
                {formState?.errors?.password && <Error message={formState.errors.password.message} />}
              </div>

              {/* rePassword */}
              <div className="mb-3">
                <label className="text-lg font-semibold" htmlFor="rePassword">Re-enter Password</label>
                <input {...register("rePassword")} type="password" className="input w-full p-2 border rounded" placeholder="re-enter password" id="rePassword" />
                {formState?.errors?.rePassword && <Error message={formState.errors.rePassword.message} />}
              </div>

              {/* dateOfBirth */}
              <div className="mb-3">
                <label className="text-lg font-semibold" htmlFor="dateOfBirth">Date of Birth</label>
                <input {...register("dateOfBirth")} type="date" className="input w-full p-2 border rounded" id="dateOfBirth" />
                {formState?.errors?.dateOfBirth && <Error message={formState.errors.dateOfBirth.message} />}
              </div>
              ...
              <p className="text-center mt-4">
                ...
                <Link to={"/login"} className="text-blue-500 font-bold ms-2">Login</Link>
              </p>
            </form>
          </div>
        </div>
